...
async function sendNotify(
  ...
  await Promise.all([
    ...
    qywxamNotifyNew(text, desp), //企业微信应用消息推送
...
async function qywxamNotifyNew(text, desp) {
    if (!QYWX_AM) return;
    const limit = 1800;
    let byteLength = Buffer.byteLength(desp);
    if (byteLength <= limit) {
        await qywxamNotify(text, desp);
        return;
    }
    
    const count = Math.ceil(byteLength / limit);
    const lines = desp.split('\n');
    const lineCount = lines.length;
    let tempLength = 0, last = 0, index = 1;
    for (let i = 0; i < lineCount; i++) {
        const line = lines[i];
        byteLength = Buffer.byteLength(line);
        if (tempLength + byteLength > limit) {
          const title = `${text} [${index++}/${count}]`;
          const content = lines.slice(last, i).join('\n');
          await qywxamNotify(title, content);
          last = i;
          tempLength = 0;
        }
    ...
    const title = `${text} [${index++}/${count}]`;
    const content = lines.slice(last, lineCount).join('\n');
    await qywxamNotify(title, content);
}
